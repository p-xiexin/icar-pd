...
public:
	 ...
	 */
	bool granaryDetection(TrackRecognition &track,
						  vector<PredictResult> predict)
	{
		...
		case GranaryStep::Enable: //[02] 粮仓使能
		{
			...
			if (granarys.size() <= 0) // 离开粮仓标志后|开始入站搜索
			{
				...
				_pointNearCone =
					searchNearestCone(track.pointsEdgeLeft, pointsCone); // 搜索右下锥桶
				if (_pointNearCone.x >
					ROWSIMAGE * 0.12) // 当车辆开始靠近右边锥桶：准备入库
				{
					counterRec++;
					if (counterRec >= 2)
					{
				...
				else if (_pointNearCone.x > 0.1 &&
						 _pointNearCone.x < ROWSIMAGE * 0.4)
				{
					slowDown = true; // 进展减速
				}
			}
		...
		{
			if (track.pointsEdgeLeft.size() >
				ROWSIMAGE / 2) // 第一阶段：当赛道边缘存在时
			{
				...
				{
					POINT startPoint =
						POINT((_pointNearCone.x + ROWSIMAGE) / 2,
							  (_pointNearCone.y + COLSIMAGE) / 2); // 线起点：右
					double k = 0, b = 0;
					k = (float)(_pointNearCone.y - startPoint.y) /
						(float)(_pointNearCone.x - startPoint.x);
			...
			else // 第二阶段：检查右下锥桶坐标满足巡航条件
			{
				...
				counterSession++;
				if ((coneRightDown.x > ROWSIMAGE / 3 &&
					 coneRightDown.y > COLSIMAGE - 80) ||
					counterSession > 20)
				{
					counterRec++;
					if (counterRec >= 2)
				...
				{
					POINT startPoint =
						POINT((coneRightDown.x + ROWSIMAGE) / 2,
							  (coneRightDown.y + COLSIMAGE) / 2); // 线起点：右
					double k = 0, b = 0;
					k = (float)(coneRightDown.y - startPoint.y) /
						(float)(coneRightDown.x - startPoint.x);
		...
		case GranaryStep::Cruise: //[04] 巡航使能
		{
			...
			// if (pointsCone.size() < 2 && track.pointsEdgeLeft.size() > ROWSIMAGE /
			// 2 && track.pointsEdgeRight.size() > ROWSIMAGE / 2)
				...
				if (indexMin != indexMax) // 开始补线
				{
					...
					if (k != 0 && b != 0)
					{
						...
						track.pointsEdgeRight =
							predictEdgeRight(repair); // 俯视域预测右边缘
			...
			else if (pointsCone.size() > 3)
			{
				...
				track.pointsEdgeRight =
					predictEdgeRight(track.pointsEdgeLeft); // 俯视域预测右边缘
			}
		...
		case GranaryStep::Exit: //[05] 出站使能
		{
			vector<POINT> pointsCone = searchCones(predict); // 计算锥桶的坐标
			POINT coneLeftUp =
				searchRightUpCone(pointsCone); // 搜索右上方的锥桶用于补线
...
private:
	...
	vector<POINT> searchCones(vector<PredictResult> predict)
	{
		...
		for (int i = 0; i < predict.size(); i++)
		{
			...
			{
				cones.push_back(POINT(predict[i].y + predict[i].height / 2,
	...
	vector<POINT> searchGranary(vector<PredictResult> predict)
	{
		...
		for (int i = 0; i < predict.size(); i++)
		{
			...
			{
				granarys.push_back(POINT(predict[i].y + predict[i].height / 2,
										 predict[i].x + predict[i].width / 2));
			}
		}
