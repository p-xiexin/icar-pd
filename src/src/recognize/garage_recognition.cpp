...
public:
	 ...
	 * @param track
	 */
	bool garageRecognition(TrackRecognition &track,
						   vector<PredictResult> predict)
	{
		...
		if (garageStep == GarageExiting || garageStep == GarageEntryingA ||
			garageStep == GarageEntryingB || garageStep == GarageEntryFinish)
			return true;
		else
			return false;
	}
	 ...
	 */
	void garageEntryRecognition(TrackRecognition &track,
								vector<PredictResult> predict)
	{
		_pointRU = POINT(0, 0);
		_pointLU = POINT(0, 0);
		...
		if (counterRec > 3 &&
			garageStep ==
				GarageEntryRecognition) // 通过AI标志提前识别车库|避免误识别环岛
		{
			garageStep = GarageEntryingA;
		}

		if (counterRec > 3 &&
			crosswalk.x > ROWSIMAGE / 2) // AI识别到斑马线：减速，精确识别入库图像
		{
			slowDown = true;
		}

		if (track.garageEnable.x ||
			garageStep == GarageStep::GarageEntryingA) // 车库识别标志
		{
			counterEnterA++;
			if (counterEnterA > 5)
			...
			_Index = "-------";
			//[02] 赛道右边缘优化
			uint16_t rowBreakLeftUp =
				searchBreakLeftUp(track.pointsEdgeLeft); // 搜索车库入库点（左上库点）
			...
			if (rowBreakLeftUp > track.garageEnable.y &&
				rowBreakLeftUp < track.pointsEdgeRight.size() - 1) // 上库点正确性校验
			{
				_Index = "1";
				uint16_t rowBreaRightDown =
					searchBreakRightDown(track.pointsEdgeRight); // 右边缘突变点（右下）
				_pointRD = track.pointsEdgeRight[rowBreaRightDown];

				if (rowBreaRightDown < track.garageEnable.y &&
					track.pointsEdgeRight[rowBreaRightDown].x >
						ROWSIMAGE * 0.75) // 远处的斑马线
				{
					_Index = "2";
					if (track.pointsEdgeRight[rowBreakLeftUp].x ==
						track.pointsEdgeRight[rowBreaRightDown].x)
						return;

					if (track.pointsEdgeRight[track.garageEnable.y].x >
						ROWSIMAGE * disGarageEntry) // 防止提前入库
					{
						double k, b;
						k = (float)(track.pointsEdgeRight[rowBreakLeftUp].y -
									track.pointsEdgeRight[rowBreaRightDown].y) /
							(float)(track.pointsEdgeRight[rowBreakLeftUp].x -
									track.pointsEdgeRight[rowBreaRightDown].x);
						b = track.pointsEdgeRight[rowBreakLeftUp].y -
							k * track.pointsEdgeRight[rowBreakLeftUp].x;

						int rowRepairDown =
							rowBreakLeftUp - (rowBreakLeftUp - track.garageEnable.y) * 2;
						if (rowRepairDown > rowBreaRightDown) // 远距离斑马线补线
						{
							_Index = "3";

							track.pointsEdgeRight[rowRepairDown].y =
								(int)(k * track.pointsEdgeRight[rowRepairDown].x + b);

							POINT startPoint =
								track.pointsEdgeRight[rowRepairDown]; // 入库补线起点：右
							POINT endPoint =
								track.pointsEdgeLeft[rowBreakLeftUp]; // 入库补线终点：左
						...
						else // 靠近斑马线
						{
							...
							else
							{
								POINT startPoint = POINT(
									ROWSIMAGE - 10, COLSIMAGE / 2); // 入库补线起点:固定左下角
								POINT endPoint =
									track.pointsEdgeLeft[rowBreakLeftUp]; // 入库补线终点
								POINT midPoint =
									POINT((startPoint.x + endPoint.x) * 0.3,
										  (startPoint.y + endPoint.y) * 0.4); // 入库补线中点
								vector<POINT> repairPoints = {startPoint, midPoint, endPoint};
								vector<POINT> modifyEdgeLeft =
									Bezier(0.02, repairPoints);			// 三阶贝塞尔曲线拟合
								track.pointsEdgeRight = modifyEdgeLeft; // 左边缘重新赋值
								track.pointsEdgeLeft = modifyEdgeLeft;
								for (int i = 0; i < modifyEdgeLeft.size();
									 i++) // 矫正右边缘坐标：最值
								{
				...
				{
					_Index = "5";
					if (track.pointsEdgeRight[rowBreakLeftUp].y -
							track.pointsEdgeLeft[rowBreakLeftUp].y <
						COLSIMAGE / 4) // 入库状态切换
					{
							...
							ROWSIMAGE * disGarageEntry) // 防止提前入库
						{
							...
							else
							{
								...
								POINT endPoint =
									track.pointsEdgeLeft[rowBreakLeftUp]; // 入库补线终点
								POINT midPoint =
									POINT((startPoint.x + endPoint.x) * 0.4,
										  (startPoint.y + endPoint.y) * 0.5); // 入库补线中点
								vector<POINT> repairPoints = {startPoint, midPoint, endPoint};
								vector<POINT> modifyEdgeLeft =
									Bezier(0.02, repairPoints);			// 三阶贝塞尔曲线拟合
								track.pointsEdgeRight = modifyEdgeLeft; // 左边缘重新赋值
		...
		if (garageStep == GarageStep::GarageEntryingA)
		{
			...
			else if (track.pointsEdgeLeft[track.pointsEdgeLeft.size() - 1].x >
						 ROWSIMAGE / 3 &&
		...
		if (garageStep == GarageStep::GarageEntryingB)
		{
			...
			else if (track.pointsEdgeLeft.size() > ROWSIMAGE * 0.7 &&
					 track.pointsEdgeRight.size() > ROWSIMAGE * 0.7)
			{
	...
	void garageEntryRec(TrackRecognition &track, vector<PredictResult> predict)
	{
		...
		if (counterRec)
		{
			...
			if (garageStep == GarageEntryRecognition)
			{
				...
				if (counterRec > 3 &&
					crosswalk.x >
						ROWSIMAGE / 4) // AI识别到斑马线：减速，精确识别入库图像
				{
					slowDown = true;
				}
			}
		...
		else if (garageStep == GarageStep::GarageEntryingB)
		{
			...
			POINT midPoint = POINT((startPoint.x + endPoint.x) / 2,
	...
	void garageExitRecognition(TrackRecognition &track)
	{
		...
		if (garageStep == GarageStep::GarageExiting) // 出库中
		{
			...
			if (track.spurroad.size() < 3 ||
				(track.stdevLeft < 100 && track.stdevRight < 100)) // 离开斑马线
			{
				counterExitOut++;
				if (counterExitOut >= 5)
			...
			if (track.pointsEdgeLeft.size() <= 1 ||
				track.pointsEdgeRight.size() <= 1) // 图像异常
				return;

			uint16_t rowBreakLeft =
				searchBreakLeft(track.pointsEdgeLeft); // 左上拐点搜索
			uint16_t rowBreakRight =
				searchBreakRight(track.pointsEdgeRight); // 右下补线点搜索

			if (track.pointsEdgeRight[rowBreakRight].x <
				ROWSIMAGE * 0.5) // 避免出库提前转向优化
			{
			...
			if (track.spurroad.size() > 2)
			{
				...
				if (startPoint.x > endPoint.x &&
					startPoint.y > endPoint.y) // 补线起点和终点正确性校验
				{
					// 斑马线右边部分补线
					midPoint = POINT((startPoint.x + endPoint.x) * 0.4,
									 (startPoint.y + endPoint.y) * 0.5); // 入库补线中点
					_pointRU = endPoint;
					vector<POINT> repairPoints = {startPoint, midPoint, endPoint};
					vector<POINT> modifyEdgeLeft =
						Bezier(0.04, repairPoints);				 // 三阶贝塞尔曲线拟合
					...
					midPoint = POINT((startPoint.x + endPoint.x) * 0.5,
									 (startPoint.y + endPoint.y) * 0.5); // 入库补线中点
					repairPoints = {startPoint, midPoint, endPoint};
	...
	void drawImage(TrackRecognition &track, Mat &trackImage)
	{
		...
		circle(trackImage, Point(_pointRU.y, _pointRU.x), 5, Scalar(226, 43, 138),
			   -1); // 上库点：紫色点
		circle(trackImage, Point(_pointRD.y, _pointRD.x), 5, Scalar(226, 43, 138),
			   -1); // 上库点：紫色点
		circle(trackImage, Point(_pointLU.y, _pointLU.x), 5, Scalar(255, 0, 255),
			   -1); // 下库点：粉色点

		putText(trackImage, _Index, Point(COLSIMAGE / 2 - 5, ROWSIMAGE - 20),
				cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 0, 255), 1, CV_AA);

		putText(trackImage, "[1] Garage", Point(COLSIMAGE / 2 - 5, 20),
		...
		putText(trackImage, step, Point(COLSIMAGE - 80, ROWSIMAGE - 20),
				cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 0, 255), 1, CV_AA);

		putText(trackImage, "Garage", Point(COLSIMAGE / 2 - 5, 20),
		...
		if (_crosswalk.x > 0)
		{
			...
			putText(trackImage, str, Point(COLSIMAGE / 2, ROWSIMAGE - 100),
					cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 0, 255), 1, CV_AA);
		}
	}
...
private:
		...
		if (pointsEdgeRight[0].y >= COLSIMAGE - 20) // 第一个点必须为右下点
		{
			for (int i = 0; i < pointsEdgeRight.size() - 50; i++) // 寻找跳变点
			{
				if (pointsEdgeRight[i].y <= pointsEdgeRight[rowBreakRight].y &&
					abs(pointsEdgeRight[i].y - pointsEdgeRight[rowBreakRight].y) < 5)
				{
					rowBreakRight = i;
					counter = 0;
				}
	...
	POINT searchCrosswalkSign(vector<PredictResult> predict)
	{
		...
		for (int i = 0; i < predict.size(); i++)
		{
			if (predict[i].label == LABEL_CROSSWALK) // 标志检测
			{
				return POINT(predict[i].y + predict[i].height / 2,
